...
export const MainPage = () => {
  ...
  useEffect(() => {
    ...
    const filterPictures = ( email: string) => {
        const filteredCards = pictures?.filter((card: any) => card.user === email)
          ...
          return (
            <div key={id} className={styles.mainPage__card}>
                <div className={styles.mainPage__infoLine}>
                    <p>{card.user}</p>
                    {typeCards === 'ALL' ?
                    <Button className={styles.mainPage__infoButton} onClick={() => filterPictures(card.user)}>Show other pictures</Button>
                    :
                    <Button className={styles.mainPage__infoButton} onClick={setAllCards}>Show all pictures</Button>
                    }
                    
                </div>
              <Image src={card.picture} alt="" />
            </div>
          );
        })}
        <Link to={PAINT_PAGE} className={styles.mainPage__paintLink}><Button text='Paint' className={styles.mainPage__paintButton}/></Link>
