...
const startDrawPoligon = (context: typeContext, setDraw: React.Dispatch<React.SetStateAction<boolean>>, ev: MouseEvent) =>
  startDraw(context, setDraw, ev, coord)

const moveDrawPoligon = (
  ev: MouseEvent,
  context: typeContext,
  ...
  for (let i = 0; i < 2 * 4; i++) {
    ...
    context?.lineTo(coord.x + radius * Math.sin(angle), coord.y + radius * Math.cos(angle))
  }
